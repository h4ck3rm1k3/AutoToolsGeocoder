...
namespace PortfolioExplorer {
	...
	///////////////////////////////////////////////////////////////////////////////
	void GeoLoadCitySoundex::Process()
	{
		...
		// Open file
		File file;
		if (!file.Open(File::CreateAndWrite, outdir + "/" + CITY_SOUNDEX_FILE, FileBufferSize))
			throw TsString("Cannot open file ") + outdir + "/" + CITY_SOUNDEX_FILE + " for output";

		// Bind local variables
		FieldAccessor stateValue = m_mapFieldAccessors["STATE"];
		FieldAccessor citySoundexValue = m_mapFieldAccessors["CITY_SOUNDEX"];
		...
		do {
			...
			// Soundex is compacted into a two-byte integer.
			TsString strSoundex = citySoundexValue.GetAsString();
			assert(strSoundex.length() == 4);
			WriteTwoByteInt(file, GeoUtil::PackSoundex(strSoundex.c_str()));

			// CityStatePostcodeID
			WriteThreeByteInt(file, cityStatePostcodeIDValue.GetAsInt());
	...
	///////////////////////////////////////////////////////////////////////////////
	std::vector<TsString> 
	GeoLoadCitySoundex::GetFieldParameters() 
	{
		std::vector<TsString> retval;

		retval.push_back("STATE");
		retval.push_back("CITY_SOUNDEX");
